...
def cordfix(towerb,towera):
    '''
    accepts two lists. tower b has its position relative to tower a. tower a is relative to the zero point
    returns the list for tower b with the corrected position relative to zero
    '''
    vectora=(float(towera[2])*math.cos(math.radians(90-float(towera[3]))),float(towera[2])*math.sin(math.radians(90-float(towera[3]))))
    vectorb=(float(towerb[2])*math.cos(math.radians(90-float(towerb[3]))),float(towerb[2])*math.sin(math.radians(90-float(towerb[3]))))
    total=(vectora[0]+vectorb[0],vectora[1]+vectorb[1])
    towerb[2]=str(round(math.hypot(total[0],total[1]),1))
    towerb[3]=str(round(90-math.degrees(math.atan2(total[1],total[0])),1))
    return towerb
...
def extract(splitlines,Callsign,Time):
    ...
    while splitlines[i] != []:
        ...
        if towerinfo[x][5]=='0':
            del towerinfo[x][5]
        elif towerinfo[x][5]=='1':
            towerinfo[x]=cordfix(towerinfo[x],towerinfo[x-1])
            del towerinfo[x][5]
        if len(towerinfo)==10:
            del towerinfo[x][-5]
        x += 1
...
def buildTable(Callsign,Time):
    if Time=='Daytime':
        ...
        timevar='Night'
    fields=[timevar+'Azimuth',timevar+'ControlRMS',timevar+'TowerRMS',timevar+'Ratio',timevar+'Theoretical',timevar+'Adjusted',timevar+'Standard',timevar+'Difference',timevar+'DifferencedB',timevar+'Augmented',timevar+'AugDifference',timevar+'AugDifferencedB']
    mergerows=[]
    basepoints=dataextract('.\\MOM Outputs\\'+Callsign+' '+Time+' Base-MN.txt')
    towerpoints=dataextract('.\\MOM Outputs\\'+Callsign+' '+Time+' Tower-MN.txt')
    faavalues=faaextract('.\\FAA Files\\'+Callsign+' '+Time+' FAA.txt')
    ...
    for i in range(len(faaangles)):
        dataset.append([5*i,basepoints[1][i],towerpoints[1][i],ratios[i],faatheopoints[1][i],resultpoints[1][i],faastanpoints[1][i],resultpoints[1][i]-faastanpoints[1][i],20*math.log10(resultpoints[1][i]/faastanpoints[1][i]),faaaugpoints[1][i],resultpoints[1][i]-faaaugpoints[1][i],20*math.log10(resultpoints[1][i]/faaaugpoints[1][i])])
    df=pandas.DataFrame(data=dataset, columns=fields)
    maximumdb=df[timevar+'DifferencedB'].max()
    maximum=df.values[df[timevar+'DifferencedB'].idxmax(),7]
    augmaximumdb=df[timevar+'AugDifferencedB'].max()
    augmaximum=df.values[df[timevar+'AugDifferencedB'].idxmax(),7]
    for i in range(72):
        for j in range(12):
            mergerows[i][fields[j]]=str(round(df.values[i,j],3))
    return mergerows,str(round(maximum,2)),str(round(maximumdb,2)),str(round(augmaximum,2)),str(round(augmaximumdb,2))
...
if running=='1' or running=='3':
    ...
    DayTable,mergeDict['DayMax'],mergeDict['DayMaxDB'],mergeDict['DayAugMax'],mergeDict['DayAugMaxDB']=buildTable(Callsign,'Daytime')
...
if running =='2' or running=='3':
    ...
    NightTable,mergeDict['NightMax'],mergeDict['NightMaxDB'],mergeDict['NightAugMax'],mergeDict['NightAugMaxDB']=buildTable(Callsign,'Nightime')
