...
The purpose of this script is, given files providing the FCC license information for a given AM radio station and the details of a
proposed cell tower nearby, generate the input files necessary to use Waterford's software to perform a MOM (Method of Moments) study.
This would be performed in a different program. The script would then take the output files generated by that software, and use it to 
create the report that would be sent to the client.
...
def prepare(file):
    '''
    Takes an FCC file and makes the necessary preparations
...
mergeDict={}
print('The filenames should be <Callsign> Daytime FCC and <Callsign> Nightime FCC')
Callsign=input('Define the callsign: ')
mergeDict['Callsign']=Callsign
if os.path.exists('.\\FCC Files\\'+Callsign+' Daytime FCC.txt') and os.path.exists('.\\FCC Files\\'+Callsign+' Nightime FCC.txt'):
